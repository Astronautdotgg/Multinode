...
function create_systemd_configuration() {
    ...
    PASS=$(date | md5sum | cut -c1-24)
		...
		cat > ${SYSTEMD_CONF}/${CODENAME}_n${NUM}.service <<-EOF
			...
			ExecStart=${MNODE_DAEMON} -daemon -pid=${MNODE_DATA_BASE}/${CODENAME}${NUM}/${CODENAME}.pid -conf=${MNODE_CONF_BASE}/${CODENAME}_n${NUM}.conf -datadir=${MNODE_DATA_BASE}/${CODENAME}${NUM} -listen=0
